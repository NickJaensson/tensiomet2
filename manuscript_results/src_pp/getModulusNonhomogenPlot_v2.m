function [] = getModulusNonhomogenPlot_v2(DataStruc, VarList, ModulusTable, X, Y, subpl)

% ModulusTable: | G | K | Kmod_fwd | Kmod_TMet | Kmod_Circ | KmodParam_TMet | KmodParam_Circ |
...
if strcmp(inputname(1), 'Wo01_Ar5_Sig60_Bal90')
    idx = find(ModulusTable(:,1) == 19);
    ModulusTable(idx,1) = 20;
end
...
if strcmp(inputname(1), 'Wo1_Ar5_Sig60_Bal80')
    idx = find(ModulusTable(:,1) == 49);
    ModulusTable(idx,1) = 50;
end
...
if strcmp(inputname(1), 'Wo1_Ar5_Sig20_Pep90')
    idx = find(ModulusTable(:,1) == 9);
    ModulusTable(idx,1) = 10;
end
...
for k = 1:size(ModulusTable,1)
    ...
    Z_TMet(i,j) = ModulusTable(k,6);
    Z_Circ(i,j) = ModulusTable(k,7);
end

subplot = @(m,n,p) subtightplot (m, n, p, [0.05 0.02], [0.1 0.2], [0.05 0.001]);
MyMap = brewermap(21, 'RdBu'); MyMap = flipud(MyMap);
...
if subpl == 4
    ...
    hCB.Ticks = [-1 -0.8 -0.6 -0.4 -0.2 0 0.2 0.4 0.6 0.8 1];
    hCB.TickLabels = [-1 -0.8 -0.6 -0.4 -0.2 0 0.2 0.4 0.6 0.8 1]*100;
    hCB.Title.String = '$K_{err}$ (\%)';
...
%Strain = round(100*(1 - DataStruc.(VarList{1,1}).Params_phys.frac));
%sgtitle(strcat('Dilatational Modulus Nonhomogeneity: $\Delta A / A$=', num2str(Strain), ...
