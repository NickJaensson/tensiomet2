function [] = getStrainApexNonhomogenPlotSTD_Noise(DataStruc, VarList, ApexStrainTable, X, Y, subpl)
...
for k = 1:size(ApexStrainTable,1)
    ...
    Z_TMet(i,j) = ApexStrainTable(k,8);
    Z_Circ(i,j) = ApexStrainTable(k,9);
end

subplot = @(m,n,p) subtightplot (m, n, p, [0.05 0.02], [0.1 0.2], [0.05 0.001]);
MyMap = brewermap(41, 'BrBG'); MyMap = flipud(MyMap); MyMap = MyMap(21:41,:);
...
if subpl == 4
    ...
    hCB.Ticks = [0 0.05 0.1 0.15 0.2 0.25];
    hCB.TickLabels = [0 0.05 0.1 0.15 0.2 0.25]*100;
    hCB.Title.String = '$\Delta \left(\lambda_{s}\lambda_{\phi}\right)_{\hat{s=0}}^{Inv}$ (\%)';
...
%Strain = round(100*(1 - DataStruc.(VarList{1,1}).Params_phys.frac));
%sgtitle(strcat('Relative error in apical strain: $\Delta A / A$=', num2str(Strain), ...
