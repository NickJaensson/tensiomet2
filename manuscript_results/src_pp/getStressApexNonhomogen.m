function [ApexStressTable] = getStressApexNonhomogen(DataStruc, VarList)
...
for i = 1:size(VarList,1)
    for j = 1:size(VarList, 2)
        ...
        ApexStress_TMet(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv.sigmas_dimal(1,1);
        ApexStress_Circ(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv.SigmaCircleFit_dimal;
    ...
    for j = 1:size(VarList, 2)-1
        if  Sigma == 20 && frac == 0.8  %excption for inaccessible geometries, average less cells
            if i == size(VarList,1)-1 
                ApexStress_fwd_mat2(i,j) = ApexStress_fwd_mat(i,j);
                ApexStress_TMet_mat2(i,j) = ApexStress_TMet_mat(i,j);
                ApexStress_Circ_mat2(i,j) = ApexStress_Circ_mat(i,j);
                StressParam_TMet_mat2(i,j) = StressParam_TMet_mat(i,j);
                StressParam_Circ_mat2(i,j) = StressParam_Circ_mat(i,j);
            else
...
%ApexStressTable = [G_vals, K_vals, ApexStress_fwd, ApexStress_TMet, ApexStress_Circ, StressParam_TMet, StressParam_Circ];
ApexStressTable = [G_vals, K_vals, ApexStress_fwd2, ApexStress_TMet2, ApexStress_Circ2, StressParam_TMet2, StressParam_Circ2];
