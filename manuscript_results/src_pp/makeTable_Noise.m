function [] = makeTable_Noise(DataStruc, NumSimuls, VarList)
...
for i = 1:size(VarList, 1)
    for j = 1:size(VarList, 2)
        ...
        K_Tensiom_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.Kmod_Avg_dimal;
        G_Tensiom_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.Gmod_Avg_dimal;
        K_Circle_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.KmodCircleFit_dimal_Avg;
        Tau_forw_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_fwd.sigmas_dimal(1,1);
        Tau_Tensiom_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.sigmas_apex_Avg_dimal(1,1);
        Tau_Circle_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.SigmaCircleFit_dimal_Avg(1,1);
        DilStr_forw_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_fwd.lams(1,1)*DataStruc.(VarList{i,j}).DefSt_fwd.lamp(1,1);
        DilStr_Tensiom_vals(idx,1) = DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.lambdas_apex_Avg*DataStruc.(VarList{i,j}).DefSt_inv{NumSimuls+1}.lambdap_apex_Avg;
