function [GK, lams, lamr] = ...
    makeSFE(strainmeasure, vars_sol_ref, vars_num_ref, vars_sol, ...
            vars_num, params_num, verbose)
    % MAKESFE Computes material parameters (shear and bulk modulus) via 
    % strain fitting.
    %
    % Uses a strain-based material model to determine the shear modulus (G) 
    % and bulk modulus (K) by comparing reference and deformed states.
    % NOTE: this code was largely adapted from Nagel "as-is" and has NOT
    % been optimized/refactored !!!
    %
    % INPUTS:
    %   strainmeasure  - Type of strain measure ('linear_hookean', 
    %                    'hencky', 'pepicelli').
    %   vars_sol_ref   - Reference state solution variables.
    %   vars_num_ref   - Reference state numerical parameters.
    %   vars_sol       - Deformed state solution variables.
    %   vars_num       - Deformed state numerical parameters.
    %   params_num     - Numerical parameters.
    %   verbose        - Boolean flag for printing iteration details.
    %
    % OUTPUTS:
    %   GK   - Estimated shear modulus (G) and bulk modulus (K) as [G, K].
    %   lams - Meridional stretch ratio.
    %   lamr - Radial stretch ratio.
